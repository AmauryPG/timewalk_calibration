...
def takeOneMeasurementDAQ(relativePathToWorkDirectory, sessionName, binToT, binToF, preampGain, selectedChannel, thresholds, vBias, laserFrequency, durationTimeSeconds):
    ...
    dict_config['daq']['thresholds'] = threshold
    ...
    filename = f"{relativePathToWorkDirectory}data_section_{selectedChannel}_Btot{binToT}_Btoa{binToF}_Gain{preampGain}_Thr{threshold}_{biais}V_Freq{laserFrequency}_{durationTimeSeconds}s.bin"

    return startTimeMeasurement, realDurationTimeInSeconds, filename


def calib_TDC(tof_data):
    pathToTDC = "../src/boardHandler/board/weeroc/Scripts_Radiopico/TDC_calib.npy"
...
def readBinaryFile(pathToBinaryFile):
...
if __name__ == "__main__":
    ...
    if(type == "number"):
        filename = sys.argv[2]
        tot_array, tof_array, size = readBinaryWeerocFile(filename)

        if len(tof_array) == 0:
            print("Error: TOF arrays is empty.")
            sys.exit(1)

        if len(tot_array) == 0:
            print("Error: TOT arrays is empty.")
            sys.exit(1)

        if len(tof_array) != len(tot_array):
            print("Error: TOF and TOT arrays have different lengths.")
            sys.exit(1)
        
        print("TOT size : ", size)
        print("Min/Max TOF : ", min(tof_array), max(tof_array))
        print("Min/Max TOT : ", min(tot_array), max(tot_array))
    elif(type == "measure"):
        sessionName = "MyMeasurementSession"
        absolutePathToWorkDirectory = os.getcwd()
        binToT = 49
        binToF = 49
        preampGain = 8
        selectedChannel = 42
        thresholdsDAC = [300]
        laserFrequency = 100000
        ...
        for file in os.listdir(path):
            if "section_2" in file and file.endswith(".bin"):
                print(f"Reading file : {file}")
                tot_array, tof_array, size = readBinaryWeerocFile(os.path.join(path, file))
                print("TOT size : ", size)
                print("Min/Max TOF : ", min(tof_array), max(tof_array))
                print("Min/Max TOT : ", min(tot_array), max(tot_array))

                threshold, biais = extractBiaisThreshold(file)

                if biais is not None and threshold is not None:
                    countList.append((biais, threshold, size))
                    print(f"Extracted biais : {biais}V, threshold : {threshold}DAC, count : {size}")

        # Write summary to a file
        with open("summary.txt", "w") as f:
            f.write("Biais (V)\tThreshold (DAC)\tCount\n")
            for biais, threshold, count in sorted(countList, key=lambda x: x[0]):
                f.write(f"{biais}\t{threshold}\t{count}\n")